...
class LayoutCraftNav {
    ...
    async handleAuthSubmit(event) {
        ...
        try {
            ...
            // Check if auth triggered by CTA button on homepage
            const postAuthPrompt = sessionStorage.getItem('layoutcraft_post_auth_prompt');
            if (postAuthPrompt) {
                // If yes, move it to the key the app page expects.
                sessionStorage.setItem('layoutcraft_initial_data', postAuthPrompt);
                sessionStorage.removeItem('layoutcraft_post_auth_prompt');
            }
            // Redirect the user to the designer page.
    ...
    setupEventListeners() {
        document.addEventListener('click', (e) => {
            ...
            if (e.target.closest('#nav-launch-app') || e.target.closest('#mobile-launch-app')) {
                e.preventDefault();
                if (!this.isLoggedIn) {
    ...
    renderMobileAuthSection() {
        if (this.isLoggedIn) {
            ...
            return `<div class="mobile-profile-section"><div class="mobile-profile-info ${planStatus.class}"><div class="profile-avatar-wrapper ${planStatus.class}"><div class="profile-avatar">${this.currentUser?.full_name?.charAt(0) || this.currentUser?.email?.charAt(0) || 'U'}</div></div><div class="mobile-profile-details"><span>${this.currentUser?.full_name || this.currentUser?.email || 'User'}</span>${mobilePlanInfo}</div></div>${planStatus.class === 'plan-free' ? '<a href="/pricing/" class="mobile-nav-link mobile-upgrade">Upgrade to Pro</a>' : ''}<a href="/account/" class="mobile-nav-link">Settings</a><button class="mobile-nav-link logout-link" type="button" data-auth-logout>Logout</button></div>`;
        } else {
    ...
    openAuthModal(mode = 'login', customSubtitle = null) {
        ...
        if (customSubtitle) {
            ...
            // Set default messages if no custom one is provided
            subtitle.textContent = mode === 'signup'
                ? 'Join LayoutCraft to unlock all features.'
